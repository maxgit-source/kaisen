...
export default function CajaRapida() {
  ...
  const productsQuery = useQuery({
    queryKey: ['productos', 'caja-rapida'],
    queryFn: async () => (await Api.productos({ all: true })) as Producto[],
    staleTime: 60_000,
  });

  const customersQuery = useQuery({
    queryKey: ['clientes', 'caja-rapida'],
    queryFn: async () => (await Api.clientes({ all: true })) as Cliente[],
    staleTime: 60_000,
  });
  ...
  useEffect(() => {
    ...
    if (defaultCustomer) setSelectedCustomerId(Number(defaultCustomer.id));
  }, [customers, selectedCustomerId]);

  useEffect(() => {
    if (!depositos.length || selectedDepositoId) return;
