...
async function createOrder(req, res) {
  ...
  try {
    const result = await withTransaction(async (client) => {
      ...
      const ids = Array.from(new Set(items.map((i) => Number(i.productId)).filter((id) => Number.isInteger(id) && id > 0)));
...
async function createOrderV2(req, res) {
  ...
  try {
    const result = await withTransaction(async (client) => {
      ...
      const placeholders = ids.map((_, idx) => `$${idx + 1}`).join(', ');
      const { rows: products } = await client.query(
        `SELECT id, name, price::float AS price, stock_quantity FROM Products WHERE id IN (${placeholders})`,
        ids
      );
      const byId = new Map(products.map((p) => [p.id, p]));
