...
async function buildRiskRanking({ limit = 100, persistSnapshot = true } = {}) {
  const baseRows = await ownerRepo.listDebtRiskBase({ limit });
...
async function generateAutoReminders({ limit = 50, userId = null } = {}) {
  const ranking = await buildRiskRanking({ limit, persistSnapshot: false });
...
async function buildRepricingPreview({ productIds = [], limit = 500 } = {}) {
  const [rules, products, dolarBlue] = await Promise.all([
    ownerRepo.listRepricingRules(),
    ownerRepo.listProductsForPricing({ productIds, limit }),
...
async function applyRepricing({ productIds = [], limit = 500, userId = null } = {}) {
  const preview = await buildRepricingPreview({ productIds, limit });
